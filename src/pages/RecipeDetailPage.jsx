...
function updateSummary(recipe) {
  ...
  const next = list.map((x) =>
    x.id === recipe.id
      ? { ...x, title: recipe.title, description: recipe.description, updatedAt: recipe.updatedAt, favorites: recipe.favorites }
      : x
  );
...
export default function RecipeDetailPage() {
  ...
  function addToGrocery() {
    ...
    writeJSON("toast.v1", { message: "Grocery list updated ✅", at: nowISO() });
  }
      ...
      <div className="grid" style={{ marginTop: "1rem" }}>
        <div className="card">
          ...
          <div className="nav-row" style={{ marginTop: ".9rem" }}>
            <button className={"btn " + (activeTab === "ingredients" ? "btn-primary" : "btn-secondary")} onClick={() => setActiveTab("ingredients")}>
              Ingredients
            </button>
            <button className={"btn " + (activeTab === "steps" ? "btn-primary" : "btn-secondary")} onClick={() => setActiveTab("steps")}>
              Steps
            </button>
            <button className={"btn " + (activeTab === "notes" ? "btn-primary" : "btn-secondary")} onClick={() => setActiveTab("notes")}>
              Notes
            </button>
          </div>
            ...
            <>
              ...
              <div style={{ marginTop: ".75rem" }}>
                {(draft.ingredients || []).map((it, idx) => (
                  <div key={it.id} style={{ borderTop: "1px solid rgba(126,224,255,.12)", paddingTop: ".75rem", marginTop: ".75rem" }}>
                    <div className="grid" style={{ gridTemplateColumns: "1.5fr .8fr .8fr", gap: ".6rem" }}>
                      <div>
                        ...
                        <input
                          className="input"
                          value={it.name}
                          onChange={(e) => updateIngredient(idx, { name: e.target.value, grocery: { ...(it.grocery || {}), canonicalName: e.target.value } })}
